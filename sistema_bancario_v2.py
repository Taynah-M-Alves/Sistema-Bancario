...
def depositar(valor_deposito):
        global limite_transacoes_diario
        if limite_transacoes_diario <= 10 :
            global saldo
            global extrato
            global horario_transacao
            limite_transacoes_diario += 1
            horario_transacao = datetime.now()
            saldo += valor_deposito
            extrato += f"{horario_transacao.strftime(mascara_ptbr)} => + R${valor_deposito:.2f}\n" # adiciona o valor depositado a lista extrato
            return print(f"Deposito realizado com sucesso! Seu saldo atual é {saldo}.")
        else:
             print("Não foi possível realizar essa transação, limite máximo de transações diárias atingido")
...
def sacar(valor_saque):
        ...
        if limite_transacoes_diario <= 10 :
            ...
            horario_transacao = datetime.now()
            saldo -= valor_saque
            extrato += f"{horario_transacao.strftime(mascara_ptbr)} => - R${valor_saque:.2f}\n" # adiciona o valor sacado a lista extrato
...
while True:
    ...
    if operacao == "a":
            ...
            excedeu_limite_saque_diario = saque_diario >= limite_saque_diario 
